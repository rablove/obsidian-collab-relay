...
export default class VaultSyncCollab extends Plugin {
  ...
  async onLocal(file) {
    ...
    // ⭐ 줄을 세워 부른다 (0.5.63) — 까닭은 upQueue 주석. 파일을 «줄 안에서» 읽는 것이 핵심이다.
  ...
  upQueue(p, fn) {
    if (!this._upQ) { this._upQ = new Map(); this._upRun = new Set(); }
    // 지금 «줄 안에서 돌고 있는» 경로를 적어 둔다 — applyRemote 가 스스로를 되부를 때 이걸 본다.
    const run = async () => { this._upRun.add(p); try { return await fn(); } finally { this._upRun.delete(p); } };
    const prev = this._upQ.get(p) || Promise.resolve();
    const next = prev.then(run, run).catch((e) => { console.error('[sync] upQueue', e); });
    this._upQ.set(p, next);
    next.then(() => { if (this._upQ.get(p) === next) this._upQ.delete(p); });   // 줄이 비면 치운다 (Map 이 안 자라게)
    return next;
  }
  ...
  docPath(doc) {
    ...
    return doc.path || ((doc._id && doc._id.indexOf(pfx) === 0) ? doc._id.slice(pfx.length) : doc._id);
  }
  ...
  async applyRemote(doc) {
    const k = nfc(this.docPath(doc) || '');
    if (this._upRun && this._upRun.has(k)) return this.applyRemoteInner(doc);
    return this.upQueue(k, () => this.applyRemoteInner(doc));
  }
